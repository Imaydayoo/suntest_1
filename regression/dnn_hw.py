...
train_set = pd.read_csv('/Users/apple/Desktop/深度学习数据/华侨医院-孕产妇EHR信息(脱敏)/train_hw.csv')
test_val_set = pd.read_csv('/Users/apple/Desktop/深度学习数据/华侨医院-孕产妇EHR信息(脱敏)/pre_hw.csv')
...
net = nn.Sequential(
    ...
    nn.Linear(12, 8),  # 第一隐层与第二隐层结点数设置，全连接结构
    torch.nn.ReLU(),  # 第一隐层激活函数采用sigmoid
    nn.Linear(8, 6),  # 第一隐层与第二隐层结点数设置，全连接结构
    torch.nn.ReLU(),  # 第一隐层激活函数采用sigmoid
    nn.Linear(6, 4),  # 第二隐层与第三隐层结点数设置，全连接结构
    torch.nn.ReLU(),  # 第二隐层激活函数采用
    nn.Linear(4, 1),  # 第三隐层与输出层层结点数设置，全连接结构
